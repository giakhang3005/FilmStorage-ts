...
export function AddNews({ setOption, isDarkMode }: Props) {
    ...
    const handleSubmit = () => {
        const now = dayjs()
        const timeString = `${now.hour()}:${now.minute()} ${now.date()}/${now.month()}/${now.year()}`
        ...
        if (!titleErr && !descriptionErr && !imageErr && !linkErr) {
            setLoading(true)
            axios.post('https://65388587a543859d1bb18661.mockapi.io/khangtng/v1/lab7_news', newNews)
                .then(() => {
                    message.success('Added successfully');
                    setOption("manageNews")
                })
                ...
                <Row className='InputContainer'>
                    ...
                    <Col xs={9}>
                        <Button
                            ...
                            type="primary" style={{ boxShadow: 'none' }}>ADD</Button>
                    </Col>
                </Row>
            </Spin>
